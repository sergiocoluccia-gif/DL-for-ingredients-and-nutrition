...
Inference code is also provided in ```run_inference_filtered.py```. The same command line parameters should be used. This assumes that the trained checkpoints are already available. We provide pretrained checkpoints (5 runs, across the different scenarios, all the networks) in the folder CHECKPOINTS (currently made available through the release files).
